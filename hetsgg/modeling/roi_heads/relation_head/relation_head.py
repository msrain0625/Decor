...
from .curriculum_scheduler import build_curriculum_scheduler
...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.curriculum_scheduler = build_curriculum_scheduler(cfg)
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.mode == "predcls":
            ...
            for proposal in proposals:
                ...
                if "pred_scores" not in proposal.extra_fields:
                    proposal.add_field("pred_scores", torch.ones(len(obj_labels)).to(device))
        ...
        if self.curriculum_scheduler is not None and 'loss_cb' in output_losses and 'curriculum_lambda' not in output_losses:
            proto = getattr(self.predictor, '_proto_cache', None)
            lam = self.curriculum_scheduler.step(proto)
            output_losses['loss_cb'] = output_losses['loss_cb'] * lam
            output_losses['curriculum_lambda'] = torch.tensor(lam, device=output_losses['loss_cb'].device)
